...
# ts draws the sharp lil triangle spikes all around the ball when spiky gets popped
# 1px black outline n filled in w the bally's own color, simple but it slaps
...
func _ready() -> void:
	z_index = 1 # sits between the ball body n the face layer
	visible = false
...
func pop_spikes() -> void:
	...
	if tween and tween.is_valid():
		tween.kill()
	tween = create_tween()
	# snappy pop out w a lil bounce to it, feels good
...
func _draw() -> void:
	...
	for i in range(spike_count):
		...
		# lil 1px black outline around the triangle so it pops visually
